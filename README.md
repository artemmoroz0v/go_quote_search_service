...
***ВАЖНО***: предварительно в pgAdmin4 я создал базу данных "Quotes". На другой машине нужно сделать так же, иначе будет ошибка и ничего ни к чему не подключится. Сама же таблица с семью полями в БД создается из программы, отдельный запрос ***create table*** писать не нужно.
